...
def analyze(job_input: str, resume_input: str) -> str:
    """Run the agent on a job posting and a resume, return the final analysis text.

    Shared by the CLI (run(), below) and the Streamlit UI (app.py) so both
    front ends drive the exact same agent instead of duplicating the logic.
    """
    ...
    result = with_retry(lambda: agent.invoke({"messages": [{"role": "user", "content": user_message}]}))
...
def run():
    ...
    answer = analyze(job_input, resume_input)
